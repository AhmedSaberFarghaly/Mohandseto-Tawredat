...
public sealed class FinanceController(FinanceService finance) : ControllerBase
{
    ...
    [HttpGet("invoices/export")] public async Task<IActionResult> Export([FromQuery] string? status, [FromQuery] DateTime? from, [FromQuery] DateTime? to, CancellationToken ct) => File(await finance.ExportAsync(status, from, to, ct), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "invoices.xlsx");
